...
const main = async () => {
  ...
  const tetherAdd = "0xdAC17F958D2ee523a2206206994597C13D831ec7";
  const swapper = "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D";

  const dai = await ethers.getContractAt("IERC20", daiAdd);
  const tether = await ethers.getContractAt("IERC20", tetherAdd);
  ...
  const liquidityResponse = await uniSwap
    .connect(impersonator)
    .addLiquidity(daiAdd, tetherAdd, first, second, 0, 0, swapper, deadline);
